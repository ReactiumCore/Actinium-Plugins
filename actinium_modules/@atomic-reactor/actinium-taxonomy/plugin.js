...
const MOD = () => {
     ...
     * ----------------------------------------------------------------------------
     */
    Actinium.Taxonomy = Actinium.Taxonomy || Taxonomy;
    ...
    Actinium.Hook.register('warning', () => {
        ...
        WARN('');
        WARN(
            chalk.cyan.bold('Notice:'),
            'The default Taxonomy has been created',
        );
    });
    ...
    Actinium.Hook.register(
        'content-retrieve',
        async (content, params, options) => {
            if (!Actinium.Plugin.isActive(PLUGIN.ID)) return;

            const { type } = content;
            const tax = await Taxonomy.Content.retrieve(
                { content, type },
                options,
            );
