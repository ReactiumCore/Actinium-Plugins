...
const MOD = () => {
    ...
    SDK.Helper.validate = (url, requiredParams) => {
        ...
        return (
            requiredParams.filter((key) => op.has(url, key)).length === count
        );
    };
    ...
    SDK.Helper.routeObject = ({ contentId, contentUUID, url, user }) => {
        ...
        urlObj.set(
            'blueprint',
            SDK.Blueprint.slug(op.get(url, 'blueprint', '')),
        );
    ...
    SDK.attach = async (params, options) => {
        ...
        let { blueprint, content, contentId, contentUUID, collection, type } =
            params;
        ...
        let routes = await qry.skip(0).limit(count).find(options);
        ...
        const attached =
            routes.length > 0
                ? await Actinium.Object.saveAll(routes, options)
                : [];
    ...
    SDK.delete = async ({ urls, ...params }, options) => {
        ...
        const deleted =
        urls.length > 0
            ? await Actinium.Object.destroyAll(urls, Actinium.Utils.MasterOptions())
            : [];
    ...
    SDK.Blueprint.update = async (params, options) => {
        ...
        const routes = await qry.skip(0).limit(count).find(options);
